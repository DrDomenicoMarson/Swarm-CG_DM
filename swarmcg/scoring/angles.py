...
def get_AA_angles_distrib(args: SwarmCGArgs, state: SwarmCGState, beads_ids):
    ...
    args/state requires:
        ...
        bw_angles
    """
    angle_values_rad = np.empty(len(state.aa2cg_universe.trajectory) * len(beads_ids))
    ...
    for ts in state.aa2cg_universe.trajectory:
        ...
        mda.lib.distances.calc_angles(bead_pos_1, bead_pos_2, bead_pos_3, backend=state.mda_backend, box=None,
                                      result=frame_values)
    ...
    angle_hist = np.histogram(angle_values_deg, state.bins_angles, density=True)[
                     0] * args.bw_angles  # retrieve 1-sum densities
...
def get_CG_angles_distrib(args: SwarmCGArgs, state: SwarmCGState, beads_ids):
    ...
    args/state requires:
        ...
        bw_angles
    """
    angle_values_rad = np.empty(len(state.cg_universe.trajectory) * len(beads_ids))
    frame_values = np.empty(len(beads_ids))
    bead_pos_1 = np.empty((len(beads_ids), 3), dtype=np.float32)
    bead_pos_2 = np.empty((len(beads_ids), 3), dtype=np.float32)
    bead_pos_3 = np.empty((len(beads_ids), 3), dtype=np.float32)
